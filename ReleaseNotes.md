...
Server-side functionality for `myfeatures` functionality has been added for testing purposes. Testing the functionality requires the `myfeatures` flywy-module to be enabled on `oskari-ext.properties` with:
```properties
...
Fixed an issue with OGC API Features "capabilities" parsing that prevented layers from being added easily by pointing to a server url: https://github.com/oskariorg/oskari-server/pull/1219
