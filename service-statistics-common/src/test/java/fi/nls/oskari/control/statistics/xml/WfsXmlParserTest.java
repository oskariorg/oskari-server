...
public class WfsXmlParserTest {
    ...
    private static String testJSONResponseKunta = ResourceHelper.readStringResource("kunnat.json",
            WfsXmlParserTest.class);
    private static String testResponseSeutukunta = ResourceHelper.readStringResource("geoserver-seutukunta.xml",
            WfsXmlParserTest.class);
    private static String testJSONResponseSeutukunta = ResourceHelper.readStringResource("seutukunnat.json",
            WfsXmlParserTest.class);

    @Test
    @Ignore("Ignored kunta string test as JSON result is a better test")
    public void testParseKuntaRegions() throws IOException, ServiceException {
    ...
    @Test
    public void testParseKuntaRegionsJSONResult() throws Exception {
        InputStream is = new ByteArrayInputStream(testResponseKunta.getBytes("UTF-8"));
        List<Region> result = WfsXmlParser.parse(is, "kuntakoodi", "kuntanimi");
        JSONArray array = new JSONArray();
        for(Region reg: result) {
            array.put(reg.toJSON());
        }
        assertTrue("JSON should match expected value", JSONHelper.isEqual(JSONHelper.createJSONArray(testJSONResponseKunta), array));
    }

    @Test
    @Ignore("Ignored seutukunta string test as JSON result is a better test")
    public void testParseSeutukuntaRegions() throws IOException, ServiceException {
    ...
    @Test
    public void testParseSeutukuntaRegionsJSONResult() throws Exception {
        InputStream is = new ByteArrayInputStream(testResponseSeutukunta.getBytes("UTF-8"));
        List<Region> result = WfsXmlParser.parse(is, "seutukuntanro", "seutukunta");
        JSONArray array = new JSONArray();
        for(Region reg: result) {
        ...
        assertTrue("JSON should match expected value", JSONHelper.isEqual(JSONHelper.createJSONArray(testJSONResponseSeutukunta), array));
    }

    @Test(expected = ServiceException.class)
    public void testParseSeutukuntaRegionsWithWrongProps() throws IOException, ServiceException {
        InputStream is = new ByteArrayInputStream(testResponseSeutukunta.getBytes("UTF-8"));
