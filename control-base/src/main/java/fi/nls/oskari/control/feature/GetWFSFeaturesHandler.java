...
public class GetWFSFeaturesHandler extends ActionHandler {
    ...
    protected SimpleFeatureCollection getFeatures(OskariLayer layer, Envelope bbox,
        ...
        try {
            ...
            throw new ActionException("Failed to decode CRSs");
        }

        if (!isWithin(targetCRS, bbox)) {
            throw new ActionParamsException("Bbox outside of targetCRS");
        }
        ...
        if (shouldTransform) {
            try {
                ...
                throw new ActionException("Failed to find CRS transformation");
            }
            try {
                requestBbox = JTS.transform(bbox, transformToRequested);
            } catch (Exception e) {
                throw new ActionException("Failed to transform bounding box");
            }
        } else {
        ...
        if (shouldTransform) {
            try {
                ...
                throw new ActionException("Failed to transform features");
            }
        }
        return features;
    }
    ...
    private Envelope parseBbox(String bbox) throws ActionParamsException {
        ...
        if (a.length != 4) {
            throw new ActionParamsException("Invalid bbox");
        }
        try {
            return new Envelope(
