...
public class OGCServices {
    ...
    /**
     * Get filter
     * @param download download details
     * @param writeParam write param
     *
     ...
     * @param writeParam write param name
     * @param download download details
     * @return BBOX filter
     */
     ...
     * @param writeParam write aram name
     * @param download download details
     * @return filter
     */
    ...
    private static String getBbox(JSONObject bbox) throws JSONException{
        return bbox.getString(PARAM_BBOX_LEFT) + ","
                + bbox.getString(PARAM_BBOX_BOTTOM) + ","
                + bbox.getString(PARAM_BBOX_RIGHT) + ","
                + bbox.getString(PARAM_BBOX_TOP);
    }

    /**
     * Get WFS Query layer plugin WFS request
     * @param download download details
     * @param addNameSpaces add namespaces?
     * @param writeParam write param
     *
        ...
        try {
            ...
            for(int id=0;id<identifiers.length();id++){
                ...
                if(filterColumnType.equals("STRING")) {
                    filter.append("<ogc:Literal>"+uniqueColumn + " LIKE '" + uniqueValue.trim() +"%" + "'</ogc:Literal>");
                } else {
     ...
     * @param download download details
     * @param addNameSpace add namespace to layer name
     * @return WFS GET feature URL
     */
    public static String doGetFeatureUrl(String wfsUrl, JSONObject download, boolean addNameSpace) throws JSONException{
