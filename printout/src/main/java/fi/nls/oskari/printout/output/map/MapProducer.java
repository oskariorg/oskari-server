...
public class MapProducer {
	...
	public MapProducer(MapProducerResource resource, String gsn, String epsg,
			...
			FactoryException, com.vividsolutions.jts.io.ParseException {
		...
		breeder.setCallback(new StatusChangeCallBack() {
			...
			public void noteException(WorkingSet workingSet,
					ChangeSet changeSet, ChangeSetEntry changeSetEntry,
	...
	private void buildLayers(
		...
		for (final LayerDefinition layerDefinition : layers) {
			...
			if ("geojson".equals(layerDefinition.getLayerType())) {
				...
				if (layerDefinition.getLayerType().equals("wfslayer")) {
					if ("false"
	...
	private void buildLayerTiles(final Vector<DirectTileLayer> mapLayers,
		...
		breeder.setTileProcessor(new MapLinkWorkingSetTileBreeder.MapLinkTileProcessor() {
			...
			/**
			 * creates Feature for Tile with required info to load tile as image
			 * 
			...
			public void processTile(Envelope e, Polygon p, int tw, int th,
				...
				for (final DirectTileLayer layer : mapLayers) {
					LayerDefinition layerDefinition = layer
							.getLayerDefinition();

					/* if predefined tiles - skip */
					if (layer.getLayerDefinition().getTiles() != null) {
						log.debug("Tile Layer - No URL processing for "
								+ layer.getLayerDefinition().getLayerid());
						continue;
					}

					DefaultFeatureCollection fc = layer.getFc();

					TileLayer tileLayer = processor.getTileLayer();

					String url = buildLayerURL(layerDefinition, e, p, tw, th,
							gridLoc, tileLayer);

					if (url == null) {
						log.debug("No URL for "
								+ layer.getLayerDefinition().getLayerid());
						continue;
					}

					++n;
					fc.add(createTileFeature(layerDefinition, url, e, tw, th, n));
	 ...
	 * @throws IOException
	 */
	String buildLayerURL(LayerDefinition layerDefinition, Envelope e,
			Polygon p, int tw, int th, long[] tileIndex, TileLayer tileLayer)
			throws IOException {
	...
	private void buildPredefinedTiles(LayerDefinition layerDefinition,
		...
		for (Map<String, ?> predefinedTile : layerDefinition.getTiles()) {
			...
			if (layerDefinition.getLayerType().equals("statslayer")) {
				...
				fc.add(createTileFeature(layerDefinition, tileUrl, e,
						Integer.MIN_VALUE, Integer.MIN_VALUE, n));

			} else {

				fc.add(createTileFeature(layerDefinition, url, e,
						Integer.MIN_VALUE, Integer.MIN_VALUE, n));
			}
	...
	protected SimpleFeature createTileFeature(
			...
			Envelope e, int tw, int th, int n) {
		gsf.setEnvelope(e);
	...
	public BufferedImage getMap(AsyncLayerProcessor asyncProc,
		...
		try {
			buildLayers(mapLayers, layers, env, transform, fcs, fcList,
					asyncProc);
