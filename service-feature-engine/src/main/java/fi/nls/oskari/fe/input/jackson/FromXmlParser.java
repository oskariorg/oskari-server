...
public class FromXmlParser extends ParserMinimalBase {
     ...
     * value, we have to create a bogus property; we will use this as the
     * property name.
     * <p>
     * Name used for pseudo-property used for returning XML Text value (which
     * does not have actual element name to use). Defaults to empty String, but
    ...
    /**
     * We need special handling to keep track of whether a value may be exposed
     * as simple leaf value.
     */
    ...
    public FromXmlParser(IOContext ctxt, int genericParserFeatures,
    ...
    /*
     * /********************************************************** /* Extended
     * API, access to some internal components
     * /**********************************************************
     */
     ...
     * {@link org.codehaus.stax2.XMLStreamWriter2} (including Typed Access API)
     * so upcasts are safe.
     */
    ...
    /**
     * Method that may be called to indicate that specified names (only local
     * parts retained currently: this may be changed in future) should be
     * considered "auto-wrapping", meaning that they will be doubled to contain
     * two opening elements, two matching closing elements. This is needed for
     * supporting handling of so-called "unwrapped" array types, something XML
     * mappings like JAXB often use.
     * <p>
     * NOTE: this method is considered part of internal implementation
     * interface, and it is <b>NOT</b> guaranteed to remain unchanged between
     * minor versions (it is however expected not to change in patch versions).
     * So if you have to use it, be prepared for possible additional work.
     * 
     * @since 2.1
     */
    ...
    public String getCurrentName() throws IOException, JsonParseException {
        ...
        // sanity check
        if (name == null) {
            throw new IllegalStateException("Missing name, in state: "
                    + _currToken);
        }
        return name;
    }
    ...
    /**
     * Since xml representation can not really distinguish between array and
     * object starts (both are represented with elements), this method is
     * overridden and taken to mean that expecation is that the current start
     * element is to mean 'start array', instead of default of 'start object'.
     */
    @Override
     ...
     * JsonParseException { JsonToken t = nextToken0(); if (t != null) { switch
     * (t) { case FIELD_NAME:
     * System.out.println("JsonToken: FIELD_NAME '"+_parsingContext
     * .getCurrentName()+"'"); break; case VALUE_STRING:
     * System.out.println("JsonToken: VALUE_STRING '"+getText()+"'"); break;
     * default: System.out.println("JsonToken: "+t); } } return t; }
     */
    ...
    public JsonToken nextToken() throws IOException, JsonParseException {
        ...
        /*
         * Need to have a loop just because we may have to eat/convert a
         * start-element that indicates an array element.
         */
        while (token == XmlTokenStream.XML_START_ELEMENT) {
            ...
            if (_parsingContext.inArray()) {
                 ...
                 * won't be reported anyway, and we need to process following
                 * event.
                 */
            ...
            /*
             * Ok: virtual wrapping can be done by simply repeating current
             * START_ELEMENT. Couple of ways to do it; but start by making
             * _xmlTokens replay the thing...
             */
            ...
            _mayBeLeaf = true;
            /*
             * Ok: in array context we need to skip reporting field names. But
             * what's the best way to find next token?
             */
        ...
        case XmlTokenStream.XML_TEXT:
            ...
            if (_mayBeLeaf) {
                ...
                /*
                 * One more refinement (pronunced like "hack") is that if we had
                 * an empty String (or all white space), and we are
    ...
    public String getValueAsString(String defValue) throws IOException,
        ...
        // into VALUE_STRING
        {
            ...
            if (str != null) {
                ...
                // note: Should NOT update context, because we will still be
                // getting
    ...
    /**
     * XML input actually would offer access to character arrays; but since we
     * must coalesce things it cannot really be exposed.
     */
    @Override
    ...
    /*
     * /********************************************************** /* Abstract
     * method impls for stuff from JsonParser
     * /**********************************************************
     */
    ...
    protected void _releaseBuffers() throws IOException {
        // anything we can/must release? Underlying parser should do all of it,
        // for now?
    }
