 ...
 * @source $URL$
 */
public class X_CurveTypeBinding extends AbstractComplexBinding implements Comparable {
    protected GeometryFactory gf;

    ArcParameters arcParameters;

    public X_CurveTypeBinding(GeometryFactory gf) {
        this.gf = gf;
    }
    ...
    public Object parse(ElementInstance instance, Node node, Object value)
        ...
        if(segments.length == 0) {
            return null;
        } else if(segments.length == 1) {  // Fork or Fix ?  - was > 1  [1]  - made by Oskari
            return segments[0];
        } else {
