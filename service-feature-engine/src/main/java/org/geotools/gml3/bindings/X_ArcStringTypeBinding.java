 ...
 * @generated
 */
public class X_ArcStringTypeBinding extends AbstractComplexBinding {

    GeometryFactory gFactory;
    CoordinateSequenceFactory csFactory;
    ArcParameters arcParameters;

    public X_ArcStringTypeBinding(GeometryFactory gFactory,
    ...
    public Object parse(ElementInstance instance, Node node, Object value)
        ...
        if (arcCoordinates.length < 3) {
            ...
            throw new RuntimeException(
                    "Number of coordinates in an arc string must be at least 3, "
                            + arcCoordinates.length + " were specified: "
                            + arcLineString);
        }
        ...
        for (int i = 0; i < arcs; ++i) {
            Coordinate c1 = arcCoordinates[i * 2 + 0];
            Coordinate c2 = arcCoordinates[i * 2 + 1];
            Coordinate c3 = arcCoordinates[i * 2 + 2];
            ...
            double tolerance = 0.1; //arcParameters.getLinearizationTolerance().getTolerance(circle);
            Coordinate[] resultSegmentCoordinates = circle.linearizeArc(c1, c2,
                    c3, tolerance);

            if (mustReverse) {
                // reverse back
                List<Coordinate> reversingCoordinates = Arrays
                        .asList(resultSegmentCoordinates);
                Collections.reverse(reversingCoordinates);
                resultSegmentCoordinates = (Coordinate[]) reversingCoordinates
                        .toArray(new Coordinate[reversingCoordinates.size()]);
            }

            resultCoordinates.addAll(Arrays.asList(resultSegmentCoordinates));
        }
        } catch( StackOverflowError re) {
            System.err.println("RE @Arc for "+node.toString());
        }
    ...
    /**
     * Returns whether the input coordinates are laid out clockwise on their
     * corresponding circle. Only works correctly if the Euclidean distance
     * between c1 and c2 is equal to the Euclidean distance between c2 and c3.
     * 
     * @param c1
     * @param c2
     * @param c3
     * @return true if input coordinates are laid out clockwise on their
     *         corresponding circle. false otherwise.
     */
    protected boolean laidOutClockwise(Coordinate c1, Coordinate c2,
            Coordinate c3) {
