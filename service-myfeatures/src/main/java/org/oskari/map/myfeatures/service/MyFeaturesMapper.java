...
public interface MyFeaturesMapper {
    ...
    @Select("SELECT id, feature_count, owner_uuid, extent, locale, fields, options, attributes FROM myfeatures_layer WHERE id = #{layerId}")
    MyFeaturesLayer findLayer(UUID layerId);

    @ResultMap("MyFeaturesLayerResult")
    @Select("SELECT id, feature_count, owner_uuid, extent, locale, fields, options, attributes FROM myfeatures_layer WHERE owner_uuid = #{ownerUuid}")
    ...
    public int insertLayer(MyFeaturesLayer layer);
    ...
    @Update("UPDATE myfeatures_layer SET "
        ...
        + "WHERE id = #{id}")
    public int updateLayer(MyFeaturesLayer layer);
    ...
    @Insert("INSERT INTO myfeatures_feature (fid, layer_id, geom, properties) VALUES (#{feature.fid}, #{layerId}, #{feature.geometry}, #{feature.properties}::json)")
    public int insertFeature(UUID layerId, MyFeaturesFeature feature);

    @Select("SELECT fid, geom, properties FROM myfeatures_feature WHERE layer_id = #{layerId} AND fid = #{fid}")
    ...
    @Select("UPDATE myfeatures_feature SET geom = #{feature.geometry}, properties = #{feature.properties} WHERE layer_id = #{layerId} AND fid = #{feature.fid}")
    ...
    @Select("SELECT fid, geom, properties FROM myfeatures_feature WHERE layer_id = #{layerId}")
    @ResultMap("MyFeaturesFeatureResult")
    public List<MyFeaturesFeature> findFeatures(UUID layerId);

    @Select("SELECT fid, geom, properties FROM myfeatures_feature WHERE layer_id = #{layerId} AND geom && ST_MakeEnvelope(#{minX}, #{minY}, #{maxX}, #{maxY})")
    @ResultMap("MyFeaturesFeatureResult")
    public List<MyFeaturesFeature> findFeaturesByBbox(UUID layerId, double minX, double minY, double maxX, double maxY);

    @Update("UPDATE myfeatures_feature SET geom = ST_FlipCoordinates(geom) WHERE layer_id = #{layerId}")
