...
public class UpdateCapabilitiesJob extends ScheduledJob {
    ...
    private void updateCapabilitiesWMS(OskariLayer layer) throws ServiceException {
        OskariLayerCapabilitiesHelper.setPropertiesFromCapabilitiesWMS(
                capabilitiesService.getCapabilities(layer, true), layer);
    }

    private void updateCapabilitiesWMTS(OskariLayer layer) throws ServiceException {
        try {
            OskariLayerCapabilitiesHelper.setPropertiesFromCapabilitiesWMTS(
                    capabilitiesService.getCapabilities(layer, true), layer, null);
        } catch (Exception e) {
            LOG.warn(e, "Failed to parse WMTS capabilities, layer: ", layer.getName());
        }
    }

    private void updateCapabilitiesWFS(OskariLayer layer) throws ServiceException {
