...
public class UpdateCapabilitiesJob extends ScheduledJob {
    ...
    protected void updateCapabilities(OskariLayer layer, Timestamp oldestAllowed) {
        ...
        String getCapabilitiesXML = getXMLFromCache(url, type, version, oldestAllowed);
        ...
        boolean success = update(getCapabilitiesXML, layer);
        // If something went wrong with the update method we don't want to save that
        if (success) {
    ...
    protected boolean canUpdate(String type) {
        return OskariLayer.TYPE_WMS.equals(type) || OskariLayer.TYPE_WMTS.equals(type);
    }

    protected String getXMLFromCache(String url, String type, String version, Timestamp oldestAllowed) {
    ...
    protected String getXMLFromService(OskariLayer layer) {
        try {
    ...
    private boolean update(String getCapabilitiesXML, OskariLayer layer) {
        switch (layer.getType()) {
        case OskariLayer.TYPE_WMS:
            return updateWMS(getCapabilitiesXML, layer);
        case OskariLayer.TYPE_WMTS:
            return updateWMTS(getCapabilitiesXML, layer);
        default:
            return false;
        }
    }
