...
public class WKTHelperTest {
    ...
    public void testGetCRS() throws Exception {
        ...
        Assertions.assertTrue(crs != null, "Got crs for proj " + WKTHelper.PROJ_EPSG_4326);

        crs = WKTHelper.getCRS(WKTHelper.PROJ_EPSG_3067);
        Assertions.assertTrue(crs != null, "Got crs for proj " + WKTHelper.PROJ_EPSG_3067);

        crs = WKTHelper.getCRS(PROJ_INVALID);
        Assertions.assertTrue(crs == null, "Null crs for invalid proj " + PROJ_INVALID);
    }
    ...
    public void testParseWKT() throws Exception {
        ...
        Assertions.assertTrue(point != null, "Got point from " + WKT_POINT);
        Assertions.assertTrue(point.getCoordinate().x == 30, "Point x correct");
        Assertions.assertTrue(point.getCoordinate().y == 10, "Point y correct");

        Geometry polygon = WKTHelper.parseWKT(WKT_POLYGON);
        Assertions.assertTrue(polygon != null, "Got polygon from " + WKT_POLYGON);
    }
    ...
    public void testTransform() throws Exception {
        ...
        Assertions.assertTrue(transformed != null, "Got point from " + transformed);
        Assertions.assertEquals(6672130.96106858, transformed.getCoordinate().y, 1e-9);
        Assertions.assertEquals(385615.37435276626, transformed.getCoordinate().x, 1e-9);
    }
    ...
    public void testCoverageTransform() throws Exception {
        ...
        Assertions.assertTrue(transformed != null, "Got result: " + transformed);
        ...
        Coordinate[] transformedCoordinates = WKTHelper.parseWKT(transformed).getCoordinates();
        Coordinate[] expectedCoordinates = WKTHelper.parseWKT(expected).getCoordinates();

        for (int i = 0; i < transformedCoordinates.length; i++) {
            Assertions.assertEquals(expectedCoordinates[i].x, transformedCoordinates[i].x, 1e-8);
            Assertions.assertEquals(expectedCoordinates[i].y, transformedCoordinates[i].y, 1e-8);
        }
    }
    ...
    public void testGetWKT() throws Exception {
        ...
        Assertions.assertEquals(WKT_POINT, str, "Should have same WKT");
    }
